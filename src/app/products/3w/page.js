...
export default function ThreeWheelers() {
  ...
  useEffect(() => {
    const loadProducts = () => {
      try {
        const saved3WProducts = localStorage.getItem("threeWheelerProducts");

        if (saved3WProducts) {
          const products = JSON.parse(saved3WProducts);
        ...
        console.error("Error loading 3-wheeler products:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    const handleUpdate = () => {
      console.log("Admin data updated - reloading 3-wheelers...");
      loadProducts();
    };
  ...
  if (models.length === 0) {
    return (
      <div className={styles.fullscreenCenter}>
        ...
        >
          <p className={styles.emptyTitle}>No 3-Wheeler Models Available</p>
  ...
  return (
    <div className={styles.pageRoot}>
      <div className={styles.container}>
        ...
        >
          <h1 className={styles.pageTitle}>3-Wheeler Collection</h1>
        ...
        <motion.button
          ...
          onClick={handleGoToTwoWheelers}
          className={`${styles.sideNavBtn} ${styles.twoWheelerBtn}`}
          aria-label="Go to 2-Wheeler Collection"
        >
        ...
        <AnimatePresence>
          {showDropdown && (
            ...
            >
              ...
              {/* Primary Specs */}
              {(current.range || current.payload || current.price) && (
                <motion.div
              ...
              {(current.motorPower || current.torque || current.driveType) && (
                <motion.div
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
